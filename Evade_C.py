...
UD = 0 # used for undrawing title4 after one session
...
while repeat:
    ...
    enemy_quantity = 75
    AI = [Enemy(win, Rectangle(point3, point4), "blue") for _ in range(enemy_quantity)]

    num_evaded = -enemy_quantity # score, after start of the game value turns into 0
    ...
    while not win.isClosed():
        ...
        for i in range(enemy_quantity):
            ...
            if AI[i].rect.gety_2() >= 500:
                num_evaded += 1
                AI[i].rect.move((AI[i].rect.getx_1() * -1), (AI[i].rect.gety_1() * -1) + random.randint(-1000, 0))
                AI[i].rect.move(random.randint(0, 800), 0)
